import { Rule, PRIORITY_XV } from "../rule";
import { Item } from "../penpaExporter";
import { BoardItem } from "puzzle-board";

type KropkiState = object;

type KropkiData = {
  horizontalBorder: number[][];
  verticalBorder: number[][];
  allShown: boolean;
};

export const kropkiRule: Rule<KropkiState, KropkiData> = {
  name: "kropki",
  initialState: {},
  initialData: (size: number) => {
    const horizontalBorder = [];
    const verticalBorder = [];

    for (let y = 0; y < size - 1; ++y) {
      horizontalBorder.push(new Array(size).fill(0));
    }
    for (let x = 0; x < size; ++x) {
      verticalBorder.push(new Array(size - 1).fill(0));
    }

    return {
      horizontalBorder,
      verticalBorder,
      allShown: true,
    };
  },
  booleanFlags: ["allShown"],
  eventTypes: ["edgeMouseDown"],
  reducer: (_state, data, event) => {
    if (event.type === "edgeMouseDown") {
      const y = event.y;
      const x = event.x;

      if (event.direction === "horizontal") {
        if (
          !(
            1 <= y &&
            y <= data.horizontalBorder.length &&
            0 <= x &&
            x < data.horizontalBorder[y - 1].length
          )
        ) {
          return {};
        }
        const newHorizontalBorder = data.horizontalBorder.map((row) => [
          ...row,
        ]);
        // Cycle: 0 (none) -> 1 (white) -> 2 (black) -> 0
        newHorizontalBorder[y - 1][x] =
          (data.horizontalBorder[y - 1][x] + 1) % 3;
        const newData = { ...data, horizontalBorder: newHorizontalBorder };
        return { data: newData };
      } else if (event.direction === "vertical") {
        if (
          !(
            0 <= y &&
            y < data.verticalBorder.length &&
            1 <= x &&
            x <= data.verticalBorder[y].length
          )
        ) {
          return {};
        }
        const newVerticalBorder = data.verticalBorder.map((row) => [...row]);
        // Cycle: 0 (none) -> 1 (white) -> 2 (black) -> 0
        newVerticalBorder[y][x - 1] = (data.verticalBorder[y][x - 1] + 1) % 3;
        const newData = { ...data, verticalBorder: newVerticalBorder };
        return { data: newData };
      }
    }
    return {};
  },
  render: (_state, data) => {
    const items: BoardItem[] = [];

    // Render horizontal borders
    for (let y = 0; y < data.horizontalBorder.length; ++y) {
      for (let x = 0; x < data.horizontalBorder[y].length; ++x) {
        if (data.horizontalBorder[y][x] !== 0) {
          const edgeY = (y + 1) * 2;
          const edgeX = x * 2 + 1;

          items.push({
            y: edgeY,
            x: edgeX,
            color: data.horizontalBorder[y][x] === 1 ? "white" : "black",
            item: "smallFilledCircle",
          });

          // Add black border to white dot for visibility
          if (data.horizontalBorder[y][x] === 1) {
            items.push({
              y: edgeY,
              x: edgeX,
              color: "black",
              item: "smallCircle",
            });
          }
        }
      }
    }

    // Render vertical borders
    for (let y = 0; y < data.verticalBorder.length; ++y) {
      for (let x = 0; x < data.verticalBorder[y].length; ++x) {
        if (data.verticalBorder[y][x] !== 0) {
          const edgeY = y * 2 + 1;
          const edgeX = (x + 1) * 2;

          items.push({
            y: edgeY,
            x: edgeX,
            color: data.verticalBorder[y][x] === 1 ? "white" : "black",
            item: "smallFilledCircle",
          });

          // Add black border to white dot for visibility
          if (data.verticalBorder[y][x] === 1) {
            items.push({
              y: edgeY,
              x: edgeX,
              color: "black",
              item: "smallCircle",
            });
          }
        }
      }
    }

    return [
      {
        priority: PRIORITY_XV,
        item: items,
      },
    ];
  },
  exportToPenpa: (data) => {
    // TODO: this is wrong, fix it later
    const items: Item[] = [];
    for (let y = 0; y < data.horizontalBorder.length; ++y) {
      for (let x = 0; x < data.horizontalBorder[y].length; ++x) {
        if (data.horizontalBorder[y][x] !== 0) {
          items.push({
            kind: "symbol",
            position: { y, x, direction: "horizontal" },
            color: data.horizontalBorder[y][x] === 1 ? 8 : 2,
            symbolName: "circle_SS",
            isFront: true,
          });
        }
      }
    }
    for (let y = 0; y < data.verticalBorder.length; ++y) {
      for (let x = 0; x < data.verticalBorder[y].length; ++x) {
        if (data.verticalBorder[y][x] !== 0) {
          items.push({
            kind: "symbol",
            position: { y, x, direction: "vertical" },
            color: data.verticalBorder[y][x] === 1 ? 8 : 2,
            symbolName: "circle_SS",
            isFront: true,
          });
        }
      }
    }

    return { items, margin: 0 };
  },
};
